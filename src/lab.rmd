...
for (file in files) {
  ...
  image_write(sample_pic, path="sample.png", format="png" )
  
  number_pic = get_number_pic(file)
  image_write(number_pic, path="number.png", format="png" )
  
  number = get_number(number_pic)
  table_sample = "sample.png" %>% pander::pandoc.image.return()
  table_number = "number.png" %>% pander::pandoc.image.return()
  samples <- rbind(samples, c(table_sample, table_number, number))
}

colnames(samples) <- c("Sample", "Number", "OCR")

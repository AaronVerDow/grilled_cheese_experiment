...
grayscale <- function(image) {
  # converts an image to grayscale without dropping alpha channel
  pixels <- image_data(image, channels="rgba")
  gray <- image_data(image, channels="gray")
...
process_number_pic <- function(image) {
  image %>% 
    image_crop(number_geometry) %>%
    image_rotate(90) %>%
    image_reducenoise() %>%
    image_modulate(saturation = 0) %>%
    image_threshold(type="white", threshold=number_threshold)  %>%
    image_threshold(type="black", threshold=number_threshold)  %>%
    image_negate() %>%
...
fake_dice <- function(id) {
  ...
  for (i in seq_along(chars)) {
    die <- image_blank(die_size, die_size, color="gray90") %>% 
      image_annotate(chars[i], size=font_size, gravity="center", font=font )
...
process_file <- function(file) {
  ...
  df <- data.frame(
    Sample = sample$path %>% pander::pandoc.image.return(),
    sample_path = sample$path,
    OCR = id$id,
    Filename = basename(file),
    Histogram = histogram$path %>% pander::pandoc.image.return(),
    Number = id$path %>% pander::pandoc.image.return(),
    Pixels = length(grays),
    Average = mean(grays),
    Standard_Deviation = sd(grays),
    Max = max(grays),
    Min = min(grays),
    ocr_comparison = id$comparison %>% pander::pandoc.image.return()
  )
